...
def Object_detection():
    ...
    corners, ids, rejected_imgpoints = aruco.detectMarkers(gray,aruco_dict,cameraMatrix,
    distCoeffs, parameters=arucoParams)
    ...
    if np.all(ids is not None):  # if there are markers found by detector
        for i in range(0, len(ids)):  # iterate in markers
            ...
            R_vec,T_vec = aruco.estimatePoseSingleMarkers(corners[i], 0.10,cameraMatrix,distCoeffs)
            print("R_vec",R_vec)
            print("T_vec",T_vec)
        ...
        det = aruco.drawDetectedMarkers(gray, corners)
        cv2.imshow('',det)
