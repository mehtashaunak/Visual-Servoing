...
class VelocityControllerPlugin1 : public ModelPlugin
{
public: VelocityControllerPlugin1() {}
public: virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
    {
        this->world = _model->GetWorld();
        this->world->SetGravity(ignition::math::Vector3d(0,0,0));

        std::cout << "Loading VelocityController Plugin1 \n";

        // Store the model pointer for convenience.
        this->model = _model;

        this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&VelocityControllerPlugin1::OnUpdate, this, _1));

        this->joints.push_back(_model->GetJoint("ur5::shoulder_pan_joint_1"));
        this->joints.push_back(_model->GetJoint("ur5::shoulder_lift_joint_1"));
        this->joints.push_back(_model->GetJoint("ur5::elbow_joint_1"));
        this->joints.push_back(_model->GetJoint("ur5::wrist_1_joint_1"));
        this->joints.push_back(_model->GetJoint("ur5::wrist_2_joint_1"));
        this->joints.push_back(_model->GetJoint("ur5::wrist_3_joint_1"));
        ...
        ros::SubscribeOptions soJointVel =
                ros::SubscribeOptions::create<ur5_vs::joint_vel>(
                    "/joint_vel_cmd_1",
                    1,
                    boost::bind(&VelocityControllerPlugin1::jointVelocityCallback, this, _1),
        ...
        ros::SubscribeOptions soJointPos =
                ros::SubscribeOptions::create<ur5_vs::joint_angles>(
                    "/joint_angles_cmd_1",
                    1,
                    boost::bind(&VelocityControllerPlugin1::jointAngleCallback, this, _1),
                    ros::VoidPtr(), &this->rosQueue);
        ...
        ros::SubscribeOptions soSimVar =
                ros::SubscribeOptions::create<ur5_vs::sim_variables>(
                    "/sim_variables_cmd_1",
                    1,
                    boost::bind(&VelocityControllerPlugin1::simVariableCallback, this, _1),
                    ros::VoidPtr(), &this->rosQueue);
        this->rosSubSimVar = this->rosNode->subscribe(soSimVar);

        this->rosJointStatePub = this->rosNode->advertise<ur5_vs::joint_states>("/my_joint_states_1",1);

        // Spin up the queue helper thread.
        this->rosQueueThread =
                std::thread(std::bind(&VelocityControllerPlugin1::QueueThread, this));
    }
...
public: void OnUpdate(const common::UpdateInfo & )
    {
        ...
        msg.ang1.data = this->joints[1]->GetAngle(0).Radian();
        msg.ang2.data = this->joints[2]->GetAngle(0).Radian();
        msg.ang3.data = this->joints[3]->GetAngle(0).Radian();
        msg.ang4.data = this->joints[4]->GetAngle(0).Radian();
        msg.ang5.data = this->joints[5]->GetAngle(0).Radian();
...
GZ_REGISTER_MODEL_PLUGIN(VelocityControllerPlugin1)
}
