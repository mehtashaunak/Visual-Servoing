...
#include <Eigen/SVD>
#include<cmath>
#include<object_detect/points_four_image_data.h>
...
double detect_points[8]={0,0,0,0,0,0,0,0};

void img_proc_callback(const object_detect::points_four_image_data  msg)
{
    x_point = msg.detected_point_x.data;
    y_point = msg.detected_point_y.data;
    centroid_depth = msg.detected_point_depth.data;
    detect_points[0]=(double)msg.detected_point_0.data;
    detect_points[1]=(double)msg.detected_point_1.data;
    detect_points[2]=(double)msg.detected_point_2.data;
    detect_points[3]=(double)msg.detected_point_3.data;
    detect_points[4]=(double)msg.detected_point_4.data;
    detect_points[5]=(double)msg.detected_point_5.data;
    detect_points[6]=(double)msg.detected_point_6.data;
    detect_points[7]=(double)msg.detected_point_7.data;
}
...
int main (int argc, char** argv)
{
    ...
    cv::Mat Error_p = cv::Mat(8, 1, CV_64F, 0.0);
    cv::Mat L = cv::Mat(8, NW, CV_64F, 0.0);
    cv::Mat L_3 = cv::Mat(2, 3, CV_64F, 0.0);
    cv::Mat Linv = cv::Mat(NW, 8, CV_64F, 0.0);
    ...
    cv::Mat UU = cv::Mat(8, 1, CV_64F, 0.0);;
    ...
    double pose[NW];
    double u_c[8]={0,0,0,0,0,0,0,0};
    double eta = 0.05; // 100 controller convergence parameter 0.03 15
    double x_t[NC],depth_p[2]={0,0},prev_depth[2], u_t[8];
    double m[8]={0,0,0,0,0,0,0,0};
    double m_t[8]={0,0,0,0,0,0,0,0};
    ...
    ros::Subscriber img_proc = nh.subscribe< object_detect::points_four_image_data>("/object_four_points", 10, img_proc_callback);
        sleep(1);
    ros::spinOnce();
...
//    UU.at<double>(1,0) = y_point; //Obtaining from img_proc_callback
    ...
    for (int i=0;i<8;i++)
    {
        UU.at<double>(i,0)=detect_points[i];
    }
 ...
 /*--------------------------------------------------------------*/
    ...
    {
        m[i]= (u_c[i]-320)/(531.15/640);    //(u-u0)/px
        m[i+1]= (u_c[i+1]-240)/(531.15/480);    //(v-v0)/py
        m_t[i]= (u_t[i]-320)/(531.15/640);    //(u-u0)/px
        m_t[i+1]= (u_t[i+1]-240)/(531.15/480);    //(v-v0)/py
    }
...
//        compute_image_jacobian_depth(m, depth_p, L);
        compute_image_jacobian_four_points(m,depth_p, L);
