...
void gen_new_node_543(int i, int bias, int n, int n_img, double sd[], double points_corner_final[], double q[], double dq[], cv::Mat &FP, std::vector< std::vector<double> > tree1, std::vector< std::vector<double> > tree2, double new_node_tree1[], double new_node_tree2[], int &obstacle, cv::Mat Rot, cv::Mat mul_Jacob,float centroid_depth,double centroid_img[], double area_current,double points_corner[]){
    ...
    //acceleration
    double dsp_mag;
    dsp_mag = sqrt(pow(dsp[0],2)+pow(dsp[1],2)+pow(dsp[2],2));
    ...
    temp3=-lambda_3*(e3/e_norm) - dsp[2];
    ...
    cout << "temp/nrm " << temp1/normv << " " << temp2/normv << " " << temp3/normv << endl;
    ...
    for (i=0; i<8; i=i+2)
    {
        uf[i]=(points_corner_final[i]-320)/(531.15/640);// using desired points
        cout << uf[i] << endl;
        uf[i+1]=(points_corner_final[i+1]-240)/(531.15/480); // using desired points
        cout << uf[i+1] << endl;
    }
    ...
    area_interaction_matrix(centroid, uf, depth_p, area_temp, L);

    //Image Jacobain inverse
    cv::invert(L, Linv, cv::DECOMP_SVD);

    //Robot Jacobian
    double q_345[6]={q[0],q[1],q[2],q[5],q[4],q[3]};
    compute_new_geo_jacobian(q_345,J);
    ...
    cout << "error (ibvs) " << err.at<double>(0,0) << " " << err.at<double>(1,0) << " " << err.at<double>(2,0) << endl;
    ...
    cout << "error_norm (ibvs) " << err_norm.at<double>(0,0) << " " << err_norm.at<double>(1,0) << " " << err_norm.at<double>(2,0) << endl;
    ...
    cout << "V1" << V_cf_lin << endl;

    cout << "V2" << V_cf_ang << endl;
