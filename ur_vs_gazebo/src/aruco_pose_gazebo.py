...
def Object_detection():
    ...
    corners, ids, rejected_imgpoints = aruco.detectMarkers(
        gray, aruco_dict, cameraMatrix, distCoeffs, parameters=arucoParams)
    ...
    if np.all(ids is not None):  # if there are markers found by detector
        for i in range(0, len(ids)):  # iterate in markers
            ...
            R_vec, T_vec, _objPoints = aruco.estimatePoseSingleMarkers(
                corners[i], 0.05, cameraMatrix, distCoeffs)
            print("R_vec", R_vec)
            print("T_vec", T_vec)
            # R_vec, T_vec, markerpoints = aruco.estimatePoseSingleMarkers(
            # corners[i], 0.02, calibration_matrix)

            # Convert euler angle to axis angle rep
            ...
            # current_position= (euler_yzx_to_axis_angle(R_vec.item(0), R_vec.item(1), R_vec.item(2)))
            # print('current_pose', current_position)
            ...
            # print(euler_yzx_to_axis_angle(R_vec.item(0),R_vec.item(1),R_vec.item(2)))
